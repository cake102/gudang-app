...
// SOLUSI 2: Ganti tipe ke React.ReactNode
const menuIcons: Record<string, React.ReactNode> = {
  Dashboard: <HiOutlineHome className="w-6 h-6" />,
  'Tambah Akun': <HiOutlineUserAdd className="w-6 h-6" />,
  'Kelola Barang': <HiOutlineClipboardList className="w-6 h-6" />,
  'Tambah Barang': <HiOutlinePlusCircle className="w-6 h-6" />,
};

export default function Sidebar({ menu }: { menu: SidebarMenu[] }) {
  ...
  return (
    <>
      ...
      >
              ...
              <li key={item.href}>
                ...
                >
                  <span className="text-xl flex-shrink-0">
                    {menuIcons[item.label] || <HiOutlineHome className="w-6 h-6" />}
                  </span>
                  <span className="transition-all duration-200 opacity-100 ml-1 whitespace-nowrap">
                    {item.label}
                  </span>
                </Link>
              </li>
        ...
        <div className="mt-auto mb-2 pb-4 w-full flex flex-col items-center gap-3">
